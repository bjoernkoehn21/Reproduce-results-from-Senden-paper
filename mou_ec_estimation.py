...
plt.figure()
FC_avg_over_subj = FC_emp[:, _I_REST_RUN, _I_NO_TIMESHIFT, :, :].mean(axis=0)
...
plt.figure()
FC_avg_over_subj = FC_emp[:, _I_REST_RUN, _I_ONE_TIMESHIFT, :, :].mean(axis=0)
plt.imshow(FC_avg_over_subj, origin='lower', cmap='Blues', vmax=1, vmin=0)
plt.colorbar()
plt.xlabel('source ROI', fontsize=14)
plt.ylabel('target ROI', fontsize=14)
...
plt.figure()
ac_avg_over_subj = np.log(np.maximum(ac[:, _I_REST_RUN, :, :].mean(axis=0),
                                     np.exp(-4.0)))
...
plt.figure()
ac_avg_over_subj = np.log(np.maximum(ac[:, _I_NBACK_RUN, :, :].mean(axis=0),
                                     np.exp(-3.1)))
plt.plot(range(n_shifts), ac_avg_over_subj)
plt.xlabel('time lag', fontsize=14)
plt.ylabel('log autocovariance', fontsize=14)
...
plt.figure()
plt.imshow(C_mod[:, _I_REST_RUN, :, :].mean(axis=0), origin='lower',
           cmap='Reds')
plt.colorbar()
plt.xlabel('source ROI', fontsize=14)
plt.ylabel('target ROI', fontsize=14)
...
confInt = np.zeros([n_runs, 2])
# Create a 1D-mask for rich club regions.
maskRichClub = np.zeros(n_rois, dtype=bool)
indexesRichClub = [23, 26, 27, 57, 60, 61]
maskRichClub[indexesRichClub] = True
print('rich club regions: '
      + str(np.concatenate(roi_labels[indexesRichClub]).tolist()))

for i_run in range(n_runs):
    # Average over subjects.
    local_var[i_run, :] = (Sigma_mod[:, i_run, :, :].mean(axis=0).diagonal())
    mean_rc_var[i_run] = local_var[i_run, maskRichClub].mean()
    mean_periph_var[i_run] = local_var[i_run, ~maskRichClub].mean()
    mean, sigma = local_var[i_run, :].mean(), local_var[i_run, :].std(ddof=1)
    confInt[i_run, :] = stt.norm.interval(0.95, loc=mean, scale=sigma)
print('95% Konfidenz Interval: ' + str(confInt))
...
maskRichClubConnections = np.zeros([n_rois, n_rois], dtype=bool)
maskRichClubConnections[indexesRichClub, indexesRichClub] = True

roiInput = np.zeros([n_runs, n_rois])
roiOutput = np.zeros([n_runs, n_rois])
ioRatioRichClub = np.zeros([n_runs])
ioRatioPeriphery = np.zeros([n_runs])
for i_run in range(n_runs):
    # Examine input-output ratio ignoring inter-rich-club connections.
    # Average over subjects.
    noRichClubConnections_C = (C_mod[:, i_run, :, :].mean(axis=0))
    noRichClubConnections_C[maskRichClubConnections] = 0
    roiInput[i_run, :] = noRichClubConnections_C[:, :].sum(axis=1)
    roiOutput[i_run, :] = noRichClubConnections_C[:, :].sum(axis=0)
    ioRatioRichClub[i_run] = (roiInput[i_run, maskRichClub].sum() /
                              roiOutput[i_run, maskRichClub].sum())
    ioRatioPeriphery[i_run] = (roiInput[i_run, ~maskRichClub].sum() /
                               roiOutput[i_run, ~maskRichClub].sum())
print('input-output ratio rich club: ', str(ioRatioRichClub))
print('input-output ratio periphery: ', str(ioRatioPeriphery))
